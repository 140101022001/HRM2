...
<body>
    ...
    <form action="{{route('HRM.update', $hrm->id)}}" method="POST">
        ...
        <div>
            <strong>性別</strong><br>
            <input type="radio" name="sex" id="option1" value="0" @if ($hrm->sex === 0) checked @endif>
            <label for="option1">
                男
            </label>
            <input type="radio" name="sex" id="option2" value="1" @if ($hrm->sex === 1) checked @endif>
            <label for="option2">
                女
            </label>
            <input type="radio" name="sex" id="option3" value="2" @if ($hrm->sex === 2) checked @endif>
            <label for="option3">
                不明
            </label>
        </div><br>
        ...
        <div>
            <strong>年齢</strong>
            <input type="number" name="age" value="{{$hrm->age}}" autocomplete="off" min="1" max="100">
        </div><br>
        <div>
            <strong>出身国</strong>
            <select name="country">
        ...
        <div>
            <strong>実施日</strong>
            <input type="date" name="first_interv_date" value="{{$hrm->first_interv_date}}" max="{{ date('Y-m-d') }}">
        </div><br>
        <div>
            <strong>対応者名</strong>
        ...
        <div>
            <strong>合否</strong><br>
            <input type="radio" name="first_interv_result" id="option1" value="0" @if ($hrm->first_interv_result === 0) checked @endif>
            <label for="option1">
                合格
            </label>
            <input type="radio" name="first_interv_result" id="option2" value="1" @if ($hrm->first_interv_result === 1) checked @endif>
            <label class="form-check-label" for="option2">
                不合格
            </label>
            <input type="radio" name="first_interv_result" id="option3" value="2" @if ($hrm->first_interv_result === 2) checked @endif>
            <label for="option3">
                未定
            </label>
        </div><br>
        ...
        <div>
            <strong>実施日</strong>
            <input type="date" name="sec_interv_date" value="{{$hrm->sec_interv_date}}" max="{{ date('Y-m-d') }}">
        </div><br>
        <div>
            <strong>対応者名</strong>
        ...
        <div>
            <strong>合否</strong><br>
            <input type="radio" name="sec_interv_result"id="option1" value="0" @if ($hrm->sec_interv_result === 0) checked @endif>
            <label for="option1">
                合格
            </label>
            <input type="radio" name="sec_interv_result" id="option2" value="1" @if ($hrm->sec_interv_result === 1) checked @endif>
            <label for="option2">
                不合格
            </label>
            <input type="radio" name="sec_interv_result" id="option3" value="2" @if ($hrm->sec_interv_result === 2) checked @endif>
            <label for="option3">
                未定
            </label>
        </div><br>
        ...
        <div>
            <strong>合否</strong><br>
            <input type="radio" name="intern_result" id="option1" value="0" @if ($hrm->intern_result === 0) checked @endif>
            <label for="option1">
                合格
            </label>
            <input type="radio" name="intern_result" id="option2" value="1" @if ($hrm->intern_result === 1) checked @endif>
            <label for="option2">
                不合格
            </label>
            <input type="radio" name="intern_result" id="option3" value="2" @if ($hrm->intern_result === 2) checked @endif>
            <label for="option3">
                未定
            </label>
        </div><br>
        ...
        <div>
            ...
            <select class="skill_se" name="skill_se[]" multiple>
                <option value="Java" @if(in_array('Java', $hrm->skill_se)) selected @endif>Java</option>
                <option value="Go" @if(in_array('Go', $hrm->skill_se)) selected @endif>Go</option>
                <option value="Kotlin" @if(in_array('Kotlin', $hrm->skill_se)) selected @endif>Kotlin</option>
                <option value="Swift" @if(in_array('Swift', $hrm->skill_se)) selected @endif>Swift</option>
                <option value="TypeScript"@if(in_array('TypeScript', $hrm->skill_se)) selected @endif>TypeScript</option>
                <option value="Ruby" @if(in_array('Ruby', $hrm->skill_se)) selected @endif>Ruby</option>
                <option value="HTML, CSS" @if(in_array('HTML,CSS', $hrm->skill_se)) selected @endif>HTML, CSS</option>
                <option value="SQL" @if(in_array('SQL', $hrm->skill_se)) selected @endif>SQL</option>
                <option value="C, C++" @if(in_array('C,C++', $hrm->skill_se)) selected @endif>C, C++</option>
                <option value="C#" @if(in_array('C#', $hrm->skill_se)) selected @endif>C#</option>
                <option value="R" @if(in_array('R', $hrm->skill_se)) selected @endif>R</option>
                <option value="Python" @if(in_array('Python', $hrm->skill_se)) selected @endif>Python</option>
                <option value="JavaScript" @if(in_array('JavaScript', $hrm->skill_se)) selected @endif>JavaScript</option>
                <option value="PHP" @if(in_array('PHP', $hrm->skill_se)) selected @endif>PHP</option>
                <option value="VBA" @if(in_array('VBA', $hrm->skill_se)) selected @endif>VBA</option>
                <option value=".NET" @if(in_array('.NET', $hrm->skill_se)) selected @endif>.NET</option>
                <option value="Objective-C" @if(in_array('Objective-C', $hrm->skill_se)) selected @endif>Objective-C</option>
                <option value="Scala" @if(in_array('Scala', $hrm->skill_se)) selected @endif>Scala</option>
                <option value="Shell Script" @if(in_array('Shell Script', $hrm->skill_se)) selected @endif>Shell Script</option>
            </select><br><br>
            <button type="button" class="btn btn-info">追加</button>
        </div><br>
        <strong>学歴</strong><br>
        <div>
            <input type="checkbox" name="graduate_4" value="1" {{ old('graduate_4', $hrm->graduate_4) == 1 ? 'checked' : '' }}>
            <label>4大</label>
        </div>
        <div>
            <input type="checkbox" name="graduate_2"  value="1" {{ old('graduate_2', $hrm->graduate_2) == 1 ? 'checked' : '' }}>
            <label>専門</label>
        </div><br>
        <div>
            <strong>最終学歴</strong>
            <input type="text" name="graduate_school" value="{{$hrm->graduate_school}}" autocomplete="off" placeholder="">
        </div><br>
        <div>
            <strong>応募職種</strong>
